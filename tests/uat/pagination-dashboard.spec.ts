...
test('paginates large lists and clarifies dashboard metrics', async ({ page }) => {
  ...
  const inventoryTable = page.getByRole('table').filter({ has: page.getByRole('columnheader', { name: 'SKU' }) })
  await expect(inventoryTable.locator('tbody tr')).toHaveCount(25)
  await expect(page.getByText(/Showing 1-25 of \d+/)).toBeVisible()
  ...
  const dashboardPeriod = page.locator('label').filter({ hasText: /^Period/ }).locator('select').first()
  await dashboardPeriod.selectOption('specific')
  ...
  await dashboardPeriod.selectOption('custom')
  await expect(page.getByLabel('From', { exact: true })).toBeVisible()
  await expect(page.getByLabel('To', { exact: true })).toBeVisible()
  await page.getByLabel('From', { exact: true }).fill('2026-06-28')
  await page.getByLabel('To', { exact: true }).fill('2026-06-28')
  await expect(page.getByText('28 Jun 2026')).toBeVisible()
...
test('shows the Speedaf remittance action on pending-payment orders', async ({ page }) => {
  ...
  // A failed/returned courier delivery must remain recoverable even while its
  // COD is awaiting remittance; returning it also prevents commission earning.
  await expect(page.getByRole('option', { name: 'Mark Returned' })).toHaveCount(1)
