                ...
                <table className="w-full text-sm">
                  ...
                  <tbody>{managementTransactions.data.map((tx: any) => {
                    ...
                    return (<tr key={tx.id} className={`border-t ${payable || approvable ? '' : 'opacity-70'}`}>
                      ...
                      <td className="px-4 py-3"><div>{tx.order_id ? <a className="text-primary hover:underline" target="_blank" rel="noreferrer" href={`/orders?order_id=${encodeURIComponent(tx.order_id)}`}>{tx.order_number}</a> : tx.order_number}</div><div className="text-xs text-muted-foreground">{tx.product_name}</div><div className="mt-1 text-xs text-muted-foreground">Earning period: {formatCommissionMonth(tx.commission_month)} · Qualified: {new Date(tx.qualification_date).toLocaleDateString()}</div>{tx.source_period && String(tx.source_period).slice(0, 7) !== String(tx.commission_month).slice(0, 7) && <div className="mt-1 text-xs font-medium text-amber-700">Late commission from {formatCommissionMonth(tx.source_period)}</div>}</td>
        ...
        <section className="space-y-5">
          <div className="rounded-lg border border-amber-300 bg-amber-50/60 p-5">
            ...
            <div className="mt-4 rounded border border-amber-300 bg-amber-100/70 p-3 text-sm text-amber-950">
              <strong>Closing confirms the approved net commission was paid outside the system.</strong> The system records the remaining approved amount as salary/payroll settled on the final day of this month, then freezes the period. Do not close until you have reviewed the entries and are ready to certify the offline payment. An administrator can safely undo the close before a later period is closed.
            </div>
            {periodReadinessLoading ? <p className="mt-4 text-sm text-muted-foreground">Preparing close preview...</p> : periodReadiness && (
              <div className={`mt-4 rounded border p-4 ${periodReadiness.isReadyToClose ? 'border-emerald-200 bg-emerald-50/60' : 'border-red-200 bg-red-50/60'}`}>
                ...
                <div className="mt-3 grid gap-3 sm:grid-cols-2 lg:grid-cols-5 text-sm"><div><span className="block text-xs text-muted-foreground">Approved credit</span>{formatMoney(periodReadiness.totalApprovedCredits || 0)}</div><div><span className="block text-xs text-muted-foreground">Deductions</span>{formatMoney(periodReadiness.totalApprovedDeductions || 0)}</div><div><span className="block text-xs text-muted-foreground">Already settled</span>{formatMoney(periodReadiness.totalSettled || 0)}</div><div><span className="block text-xs text-muted-foreground">Marked paid on close</span>{formatMoney(periodReadiness.totalUnpaid || 0)}</div><div><span className="block text-xs text-muted-foreground">Recovery due next month</span>{formatMoney(periodReadiness.totalRecovery || 0)}</div></div>
            ...
            <form className="mt-4 grid gap-3 md:grid-cols-[minmax(0,1fr)_minmax(0,2fr)_auto]" onSubmit={async event => {
              ...
              if (!confirm(`Close ${periodLabel} and confirm ${formatMoney(periodReadiness?.totalUnpaid || 0)} was paid outside the system? ${formatMoney(periodReadiness?.totalRecovery || 0)} recovery will remain due.`)) return
              ...
              <div className="flex items-end"><button disabled={closePeriodMutation.isPending || !periodReadiness?.isReadyToClose} title={!periodReadiness?.isReadyToClose ? 'Resolve the close-preview blockers first' : 'Close and certify offline payroll payment'} className="w-full rounded bg-amber-700 px-4 py-2 text-sm font-medium text-white disabled:cursor-not-allowed disabled:opacity-50">{closePeriodMutation.isPending ? 'Closing and recording payment...' : periodReadiness?.isReadyToClose ? 'Close & confirm paid' : periodReadiness?.pendingCount ? `Review ${periodReadiness.pendingCount} pending first` : 'Resolve close blockers first'}</button></div>
            </form>
            ...
            {closePeriodMutation.isSuccess && <p className="mt-3 text-sm text-emerald-700">Period closed and its approved net commission recorded as paid through offline payroll.</p>}
          </div>

          <div className="rounded-lg border bg-card overflow-hidden">
            <div className="px-4 py-3 border-b">
              <h3 className="font-semibold">Commission period closure history</h3>
              <p className="text-xs text-muted-foreground">Each close records the approved, deducted and externally settled totals. A safe administrative undo remains visible here and in the audit log.</p>
            </div>
                ...
                {periodClosures.closures.map((closure: any) => (
                  <details key={closure.id} className="group">
                    <summary className="flex cursor-pointer list-none flex-wrap items-center justify-between gap-3 px-4 py-3 hover:bg-muted/40">
                      ...
                      <div className="flex flex-wrap gap-3 text-xs text-muted-foreground"><span>Outstanding after close: {formatMoney(closure.totalUnpaid || 0)}</span><span>Recovery: {formatMoney(closure.totalRecovery || 0)}</span><span>{closure.status === 'reopened' ? `Reopened ${formatCommissionTimestamp(closure.reopenedAt)}` : `Closed ${formatCommissionTimestamp(closure.closedAt || closure.closed_at)}`}</span></div>
                    </summary>
                    <div className="border-t bg-muted/20 px-4 py-4 text-sm">
                      <dl className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
                        ...
                        <div><dt className="text-xs text-muted-foreground">Outstanding after close</dt><dd className="font-medium">{formatMoney(closure.totalUnpaid || 0)}</dd></div>
                        <div><dt className="text-xs text-muted-foreground">Recovery due</dt><dd className="font-medium">{formatMoney(closure.totalRecovery || 0)}</dd></div>
                      </dl>
