...
RELEASE_ID="admin-order-status-correction-release-20260824"
MIGRATION="admin_order_status_correction_migration.sql"
...
if ! db_psql -tAc "SELECT to_regclass('public.orders') IS NOT NULL AND to_regclass('public.journal_entries') IS NOT NULL" | grep -qx t; then
  echo "The required Dlight order and accounting schema was not found."
  exit 1
fi
...
function_is_current="$(db_psql -tAc "SELECT POSITION('sale_recognized_' IN pg_get_functiondef('account_completed_order()'::regprocedure)) > 0" | tr -d '[:space:]')"
...
elif [ "$function_is_current" = "t" ]; then
  echo "The correction-aware accounting function exists but ${MIGRATION} is not recorded. Refusing to guess a partially applied state."
  exit 1
else
...
verified="$(db_psql -tAc "SELECT
  POSITION('sale_recognized_' IN pg_get_functiondef('account_completed_order()'::regprocedure)) > 0
  AND EXISTS (SELECT 1 FROM pg_trigger WHERE tgname='trg_account_completed_order' AND NOT tgisinternal)
" | tr -d '[:space:]')"
if [ "$verified" != "t" ]; then
  echo "Post-migration order-status accounting verification failed."
  exit 1
fi

echo "Order-status correction migration completed and verified."
