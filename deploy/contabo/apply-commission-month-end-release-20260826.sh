...
RELEASE_ID="commission-month-end-release-20260826"
MIGRATION="commission_month_end_usability_migration.sql"
...
if ! db_psql -tAc "SELECT to_regclass('public.commission_payments') IS NOT NULL AND to_regclass('public.commission_period_closures') IS NOT NULL" | grep -qx t; then
  echo "The required commission schema was not found. Apply the original commission release first."
  exit 1
fi
...
if compose ps --status running --services | grep -qx app; then
  ...
  echo "Stopping the application before backup and migration."
  compose stop app
fi
...
schema_is_current="$(db_psql -tAc "SELECT
  EXISTS (SELECT 1 FROM pg_enum e JOIN pg_type t ON t.oid=e.enumtypid WHERE t.typname='payment_method' AND e.enumlabel='payroll')
  AND EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='commission_payments' AND column_name='voided_at')
  AND EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='commission_period_closures' AND column_name='reopened_at')
" | tr -d '[:space:]')"
...
elif [ "$schema_is_current" = "t" ]; then
  echo "The month-end schema exists but ${MIGRATION} is not recorded. Refusing to guess a partially applied state."
  exit 1
else
...
verified="$(db_psql -tAc "SELECT
  EXISTS (SELECT 1 FROM pg_enum e JOIN pg_type t ON t.oid=e.enumtypid WHERE t.typname='payment_method' AND e.enumlabel='payroll')
  AND EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='commission_payments' AND column_name='void_reason')
  AND EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='commission_period_closures' AND column_name='reopen_reason')
  AND POSITION('dlight.commission_reopen_closure' IN pg_get_functiondef('guard_closed_commission_period_closure()'::regprocedure)) > 0
" | tr -d '[:space:]')"
if [ "$verified" != "t" ]; then
  echo "Post-migration commission month-end verification failed."
  exit 1
fi

echo "Commission month-end migration completed and verified."
